...
// 比如从内部讨论中获取课堂政策等。
//
// 课堂讨论分散在很多聊天记录中（群聊与单聊），
// 本示例演示完整的知识提取链路：
// 1. 通过 `connect::chat::LarkChatFetcher` 跨群搜索「课堂」讨论
// 2. 通过 `connect::extract` 用 LLM 从聊天记录中提取课堂政策
...
/// 课堂政策提取提示词（LLM 使用）
const POLICY_PROMPT: &str = r#"你是一个课堂政策提取工具。从聊天记录中提取量潮课堂相关的政策与规则。

输出 JSON：
1. policies: 逐条政策（name, description, source），source 为原始消息片段
2. summary: 一句话总结当前课堂政策方向"#;

fn main() -> anyhow::Result<()> {
    // 1. 通过 connect 模块跨群搜索「课堂」聊天记录（lark-cli im +messages-search）
    let fetcher = LarkChatFetcher;
    let msgs = fetcher.search("课堂")?;

    println!(
        "=== 课堂讨论（connect 模块跨群搜索，共 {} 条，展示前 10 条）===\n",
        msgs.len()
    );
    ...
    // 2. 通过 connect::extract 用 LLM 提取课堂政策
    let discussion = msgs
        .iter()
    ...
    // 3. 保存到本地数据目录（data/qtclass，与 qtrecurit 示例隔离）
    let output_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("data")
        .join("qtclass");
    fs::create_dir_all(&output_dir)?;
    let yaml = serde_yaml::to_string(&result)?;
    let output_path = output_dir.join("class_policies.yaml");
